...
async function load() {
  ...
  try {
    ...
    data.value = await fetchHistoryList(page.value, pageSize, q);
...
async function openDetail(entry: HistoryEntry) {
  ...
  try {
    const raw = await getHistoryResult(entry.id);
    detailResult.value = JSON.parse(raw) as PipelineResult;
  } catch (e: any) { message.error("加载详情失败: " + String(e)); }
  finally { detailLoading.value = false; }
}

const aiContent = computed(() => {
  const md = detailResult.value?.markdown;
  if (!md) return "";
...
async function copyDetail() {
  ...
  try {
    await writeText(`【${detailEntry.value.title}】 ${detailEntry.value.url}\n\n${aiContent.value}`);
    message.success("已复制");
...
async function exportDetail() {
  ...
  try {
    ...
    if (path) {
      ...
      await invoke("save_result_to_file", { result: detailResult.value, outputPath: path });
      message.success("导出成功");
    }
  } catch (e: any) { message.error("导出失败: " + String(e)); }
}

async function doDelete(entry: HistoryEntry) {
  try { await deleteHistoryItem(entry.id); load(); }
  catch (e: any) { message.error("删除失败: " + String(e)); }
}

async function doClearAll() {
  try { await clearHistory(); data.value = null; message.success("已清空"); }
  catch (e: any) { message.error("清除失败: " + String(e)); }
}
...
<template>
  <div class="history-root">
    ...
    <div class="history-list" v-if="data && data.entries.length > 0">
      <div
        v-for="entry in data.entries" :key="entry.id"
        class="h-entry"
        :class="{ 'h-done': entry.status === 'done', 'h-err': entry.status === 'error' }"
      >
        <!-- Left: cover thumbnail -->
        <div class="h-thumb" @click="openDetail(entry)">
          <img v-if="entry.cover" :src="entry.cover + '@160w_100h_1c'" class="h-cover" referrerpolicy="no-referrer" />
          <div v-else class="h-cover-fb">
            <n-icon size="22" color="#bbb"><EyeOutline /></n-icon>
          </div>
        </div>

        <!-- Center: text content -->
        <div class="h-body" @click="openDetail(entry)">
          ...
          <div class="h-line2">
            ...
            <span v-if="entry.uploader" class="h-dot">·</span>
        ...
        <!-- Right: actions -->
        <div class="h-right">
          <div class="h-status-line">
            <n-icon v-if="entry.status === 'done'" color="#18a058" size="15"><CheckmarkCircle /></n-icon>
            <n-icon v-else color="#d03050" size="15"><CloseCircle /></n-icon>
            <span class="h-elapsed">{{ fmtElapsed(entry.elapsed_ms) }}</span>
          </div>
          <n-button size="tiny" text @click="openDetail(entry)" class="h-action-btn">
            <template #icon><n-icon size="15"><EyeOutline /></n-icon></template>
          </n-button>
          <n-popconfirm @positive-click="doDelete(entry)">
            <template #trigger>
              <n-button size="tiny" text type="error" class="h-action-btn">
    ...
    <n-drawer v-model:show="detailDrawer" width="720">
      <n-drawer-content closable>
        <template #header>
          <span style="font-size:15px;font-weight:600;">{{ detailEntry?.title ?? '详情' }}</span>
        </template>
