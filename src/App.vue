...
import { NInput, NButton, NSpace, NProgress, NText, NIcon, createDiscreteApi, NDrawer, NDrawerContent, NTabs, NTabPane, NTag, NSelect } from "naive-ui";
...
const showSettings = ref(false), resultTab = ref("markdown");
  ...
  <div class="app-root">
    ...
    <main class="app-main">
      ...
      <div v-if="store.result" class="result-area">
        <div class="result-header"><n-text strong class="result-title">{{ store.result.video_info.title }}</n-text><n-button size="small" @click="store.exportToFile()"><template #icon><n-icon><DownloadOutline /></n-icon></template>Export MD</n-button></div>
        <n-tabs v-model:value="resultTab" type="line" class="result-tabs">
          <n-tab-pane name="markdown" tab="Markdown"><template #tab><n-space :size="4" align="center"><n-icon size="15"><DocumentTextOutline /></n-icon><span>Markdown</span></n-space></template><div class="tab-toolbar"><n-button size="tiny" quaternary @click="copyText(store.result.markdown,'Markdown')"><template #icon><n-icon><CopyOutline /></n-icon></template>Copy</n-button></div><div class="tab-scroll-native"><div class="md-preview" v-html="renderMarkdown(store.result.markdown)" /></div></n-tab-pane>
          <n-tab-pane name="transcript" tab="Transcript"><template #tab><n-space :size="4" align="center"><n-icon size="15"><DocumentTextOutline /></n-icon><span>Transcript</span></n-space></template><div class="tab-toolbar"><n-button size="tiny" quaternary @click="copyText(store.result.transcript,'Transcript')"><template #icon><n-icon><CopyOutline /></n-icon></template>Copy</n-button></div><div class="tab-scroll-native"><pre class="transcript-text">{{ store.result.transcript }}</pre></div></n-tab-pane>
          <n-tab-pane name="insights" tab="Insights"><template #tab><n-space :size="4" align="center"><n-icon size="15"><BulbOutline /></n-icon><span>Insights</span></n-space></template><div class="tab-toolbar"><n-button size="tiny" quaternary @click="copyText(JSON.stringify(store.result.insights,null,2),'Insights')"><template #icon><n-icon><CopyOutline /></n-icon></template>Copy JSON</n-button></div><div class="tab-scroll-native"><div class="insights-block"><div class="insight-section"><span class="insight-label">Summary</span><p>{{ store.result.insights.summary }}</p></div><div class="insight-section"><span class="insight-label">Key Points</span><ol><li v-for="(pt,i) in store.result.insights.key_points" :key="i">{{ pt }}</li></ol></div><div class="insight-section"><span class="insight-label">Tags</span><div class="tag-row"><n-tag v-for="(t,i) in store.result.insights.tags" :key="i" size="small">{{ t }}</n-tag></div></div></div></div></n-tab-pane>
          <n-tab-pane name="info" tab="Info"><template #tab><n-space :size="4" align="center"><n-icon size="15"><InformationCircleOutline /></n-icon><span>Info</span></n-space></template><div class="tab-scroll-native"><table class="info-table"><tr><td>BV</td><td><code>{{ store.result.video_info.bvid }}</code></td></tr><tr><td>Title</td><td>{{ store.result.video_info.title }}</td></tr><tr><td>Uploader</td><td>{{ store.result.video_info.uploader }} (UID:{{ store.result.video_info.uploader_uid }})</td></tr><tr><td>Duration</td><td>{{ fmtDur(store.result.video_info.duration) }}</td></tr><tr><td>Published</td><td>{{ new Date(store.result.video_info.pubdate*1000).toLocaleString() }}</td></tr><tr v-if="store.result.video_info.description"><td>Description</td><td class="desc-cell">{{ store.result.video_info.description }}</td></tr></table></div></n-tab-pane>
        </n-tabs>
      </div>
...
.tab-scroll-native{flex:1;min-height:0;overflow-y:auto;overflow-x:hidden}

.transcript-text{margin:0;white-space:pre-wrap;padding:0 20px;line-height:1.8;font-size:14px;color:#333;font-family:inherit}
.insights-block{padding:4px 20px}.insight-section{margin-bottom:18px}.insight-label{font-size:12px;font-weight:600;color:#999;display:block;margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px}.insight-section p{line-height:1.7;color:#333}.insight-section ol{margin:0;padding-left:20px}.insight-section ol li{line-height:1.7;margin:3px 0}.tag-row{display:flex;flex-wrap:wrap;gap:6px}
.info-table{width:calc(100% - 40px);margin:0 20px;border-collapse:collapse;font-size:13px}.info-table td{padding:7px 0;vertical-align:top}.info-table td:first-child{color:#999;width:90px;white-space:nowrap}.info-table code{font-size:12px;background:#f5f5f5;padding:1px 6px;border-radius:3px}.desc-cell{line-height:1.6;color:#555}
