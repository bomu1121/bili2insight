...
import {
  ...
  NDrawer, NDrawerContent, NTabs, NTabPane, NTag, NSelect,
} from "naive-ui";
import {
  PlayOutline, DownloadOutline, CopyOutline, SettingsSharp,
  VideocamOutline, DocumentTextOutline, BulbOutline, InformationCircleOutline,
...
function renderMarkdown(text: string) {
  let h = text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/^### (.+)$/gm, '<h3>$1</h3>').replace(/^## (.+)$/gm, '<h2>$1</h2>').replace(/^# (.+)$/gm, '<h1>$1</h1>').replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>').replace(/`(.+?)`/g, '<code>$1</code>').replace(/^- (.+)$/gm, '<li>$1</li>').replace(/^(\d+)\. (.+)$/gm, '<li>$2</li>').replace(/^---$/gm, '<hr>').replace(/\n\n/g, '</p><p>').replace(/\n/g, '<br>');
  return '<p>' + h + '</p>';
}
  ...
  <n-layout style="height: 100vh; background: #f5f5f5;">
    ...
    <n-layout-content class="main-content">
      ...
      <div v-if="store.preview" class="preview-card">
        <div class="preview-cover" v-if="store.preview.cover">
          <img :src="store.preview.cover" style="width:120px;height:68px;object-fit:cover;border-radius:4px;" />
        </div>
        <div class="preview-info">
          <n-text strong style="font-size: 14px;">{{ store.preview.title }}</n-text>
          <n-text depth="3" style="font-size: 12px;">{{ store.preview.uploader }} &middot; {{ fmtDur(store.preview.duration) }}</n-text>
        </div>
      </div>
    ...
    <n-drawer v-model:show="showSettings" width="400">
      <n-drawer-content title="Settings" closable>
        <n-space vertical style="gap: 16px;">
          ...
          <div><n-text depth="3" style="font-size:12px;">AI Provider</n-text><n-select v-model:value="store.selectedProvider" :options="store.PROVIDERS.map((p,i)=>({label:p.name,value:i}))" size="small" @update:value="(i:number)=>store.switchProvider(i)" /></div>
          ...
          <div v-if="store.PROVIDERS[store.selectedProvider].models.length > 0"><n-text depth="3" style="font-size:12px;">Model</n-text><n-select v-model:value="store.aiModel" :options="store.PROVIDERS[store.selectedProvider].models.map(m=>({label:m,value:m}))" size="small" /></div>
