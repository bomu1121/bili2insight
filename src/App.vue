...
async function doSendSms() {
  if (!smsPhone.value) { message.warning("Please enter phone number"); return; }
  smsSending.value = true;
  try {
    // For now, SMS send requires valid captcha token which needs Geetest integration
    message.info("SMS login requires captcha verification which is being implemented");
...
<template>
  <div class="app-root">
    <header class="app-header">
      ...
      <n-space :size="8">
        <n-button text @click="showQueue = true" style="position:relative;">
          <template #icon><n-icon><ListOutline /></n-icon></template>Queue
          <span v-if="store.queueCount > 0" class="hdr-badge">{{ store.queueCount }}</span>
        </n-button>
        <div class="hdr-avatar" @click="openLogin" :title="store.isLoggedIn ? store.loginUname : 'Login'">
    ...
    <n-drawer v-model:show="showQueue" width="420" placement="right">
      <n-drawer-content title="Queue" closable>
        <div class="queue-drawer" v-if="store.queue.length > 0">
          <div class="queue-actions">
            ...
            <n-button size="small" @click="clearDone" :disabled="store.queue.filter(q=>q.status==='done'||q.status==='error').length===0">
              <template #icon><n-icon><TrashOutline /></n-icon></template>Clear done
            </n-button>
          </div>
          <div class="queue-list">
            <div v-for="item in store.queue" :key="item.id" class="q-item" :class="{ running: item.status === 'running', done: item.status === 'done', error: item.status === 'error' }">
              ...
              <div class="q-info">
                ...
                <n-text depth="3" style="font-size:11px;">P{{ item.pageInfo.page }} &middot; {{ item.status === 'done' ? 'Done' : item.status === 'error' ? 'Failed' : item.status === 'running' ? item.stageLabel : 'Pending' }}</n-text>
        ...
        <n-text depth="3" v-else style="display:block;text-align:center;padding:60px 0;">Queue is empty</n-text>
      </n-drawer-content>
    </n-drawer>

    <!-- Login Drawer -->
    <n-drawer :show="store.showLogin" @update:show="(v) => { if (!v) store.cancelLogin(); }" width="400">
      <n-drawer-content title="Bilibili Login" closable>
        <!-- Logged-in state -->
        <div class="login-body" v-if="store.isLoggedIn">
          <div class="login-success">
            ...
            <n-button type="error" size="small" @click="store.doLogout()" style="margin-top:18px;">
              <template #icon><n-icon><LogOutOutline /></n-icon></template>Logout
            </n-button>
          </div>
        </div>
        ...
        <!-- Login tabs: QR + SMS -->
        <div v-else>
          <n-tabs v-model:value="qrTab" type="line" animated>
            <n-tab-pane name="qr" tab="QR Code">
              <template #tab>
                <n-icon size="18"><QrCodeOutline /></n-icon>
                <span style="margin-left:6px;">QR Code</span>
              </template>
            ...
            <n-tab-pane name="sms" tab="SMS">
              <template #tab>
                <n-icon size="18"><PhonePortraitOutline /></n-icon>
                <span style="margin-left:6px;">SMS</span>
              </template>
              <div class="tab-content">
                <div class="sms-section">
                  <n-input v-model:value="smsPhone" placeholder="Phone number" size="large" style="width:100%;" />
                  <n-space style="width:100%;margin-top:12px;" :size="8">
                    <n-input v-model:value="smsCode" placeholder="SMS code" size="large" style="flex:1;" />
                    <n-button type="primary" @click="doSendSms" :disabled="smsSending || smsCountdown > 0" :loading="smsSending" style="flex-shrink:0;">
                      {{ smsCountdown > 0 ? smsCountdown + 's' : smsSent ? 'Resend' : 'Send' }}
                    </n-button>
                  </n-space>
                  <n-button type="primary" block @click="message.info('SMS login needs captcha - coming soon')" style="margin-top:16px;">
                    <template #icon><n-icon><ArrowForward /></n-icon></template>Login
                  </n-button>
                </div>
              </div>
            </n-tab-pane>
    ...
    <!-- Settings Drawer -->
    <n-drawer v-model:show="showSettings" width="440"><n-drawer-content title="Settings" closable><n-space vertical style="gap:16px;">
      ...
      <div><n-text depth="3" style="font-size:12px;">AI Provider</n-text><n-select v-model:value="store.selectedProvider" :options="store.PROVIDERS.map((p,i)=>({label:p.name,value:i}))" size="small" @update:value="(i) => store.switchProvider(i)" /></div>
      <div><n-text depth="3" style="font-size:12px;">API URL</n-text><n-input v-model:value="store.aiApiUrl" size="small" /></div>
      <div>
        <n-text depth="3" style="font-size:12px;">API Key</n-text>
        <n-space :size="6" style="margin-top:4px;flex-wrap:nowrap;">
          ...
          <n-button size="small" @click="store.fetchModelList()" style="flex-shrink:0;white-space:nowrap;">Test &amp; Fetch</n-button>
        </n-space>
      </div>
      <div v-if="store.customModels.length>0||store.PROVIDERS[store.selectedProvider].models.length>0">
        <n-text depth="3" style="font-size:12px;">Model</n-text><n-select v-model:value="store.aiModel" :options="(store.customModels.length>0?store.customModels:store.PROVIDERS[store.selectedProvider].models).map(m=>({label:m,value:m}))" size="small" />
      </div>
      <div v-else><n-text depth="3" style="font-size:12px;">Model</n-text><n-input v-model:value="store.aiModel" size="small" /></div>
      <div>
        <n-space justify="space-between" align="center"><n-text depth="3" style="font-size:12px;">Prompt Template</n-text><n-button size="tiny" @click="store.addCustomTemplate()">+ New</n-button></n-space>
      ...
      <div v-if="store.selectedTemplateIndex >= store.BUILTIN_TEMPLATES.length">
        <n-text depth="3" style="font-size:12px;">Template Name</n-text>
      ...
      <div>
        <n-text depth="3" style="font-size:12px;">Prompt Content{{ store.selectedTemplateIndex < store.BUILTIN_TEMPLATES.length ? ' (builtin, read-only)' : '' }}</n-text>
